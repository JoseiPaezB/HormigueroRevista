...
const ContentComponent = ({ contentType }) => {
  ...
  // Fetch data on component mount - NOW DYNAMIC
  useEffect(() => {
    const handleResize = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        // 1. Fetch revista - dynamic based on URL param or latest
        if (edicionId) {
        ...
        const { data: contentData, error: contentError } = await supabase
          .from('creaciones')
          .select('*')
          .eq('id_revista', revistaData.id) // Use dynamic revista ID
          .eq('tipo', contentType)
          .single();
  ...
  }, [contentType, edicionId]); // Re-fetch when contentType or edicionId changes
  ...
  const getAuthorsNames = () => {
    ...
    return contributors.slice(0, 8).join(', ');
  };
            ...
            <div className="contributors-list" style={{
              ...
              {contributors.length > 0 ? (
                ...
                <ScrollReveal delay={100} direction="left">
                  <p style={{marginBottom: '10px'}}>Cargando autores...</p>
                </ScrollReveal>
              )}
            </div>
          </ScrollReveal>
